...
class EnhancedMarkdownFile:
    ...
    def clear_error_message(self, kind: str = "markdown") -> None:
        """Clear the error message from cache and file."""
    ...
    def _material_template_error_message(
        self,
        error_message: str,
        filepath: Path,
    ) -> str:
        ...
        # if the content matches the pattern, extract the metadata and prepend
        # it to the content
    ...
    def read_cached_markdown(self) -> Optional[str]:
        ...
        if is_cached and not self.changed_since_last_run():
            log.info(
                f"{Fore.YELLOW}> no changes detected in {Fore.GREEN}{self}{Style.RESET_ALL}"
            )
    ...
    def generate_markdown_output(self, save: bool = True) -> Optional[str]:
        try:
            ...
            log.error(f"Failed to read cached markdown: {e}")
            return None

        try:
            # 1. Run Juvix Markdown if the file is a Juvix Markdown file otherwise
            # just read the file
            ...
            markdown_output = process_images(
                self.env, markdown_output, self.absolute_filepath
            )
            if markdown_output is None:
                return None
            ...
            # 3. Extract metadata
            # take advantage of the fact that the markdown output is cached
            self._metadata = self._parse_front_matter(markdown_output)
            self._metadata = None
        except Exception as e:
            log.error(f"Failed to generate markdown output: {e}")
            return None
        ...
        if save:
            try:
                ...
                log.error(f"Failed to save markdown output: {e}")
    ...
    def metadata(self) -> Optional[dict]:
        ...
        if self.markdown_output is None:
            return None
        self._metadata = self._parse_front_matter(self.markdown_output)
        return self._metadata
    ...
    def has_changes(self) -> bool:
        try:
            ...
            if self.hash != self._stored_hash:
                ...
                log.info("No changes detected in Markdown files.")
                return False
        except Exception as e:
