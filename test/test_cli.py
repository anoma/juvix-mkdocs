...
def test_new_command_existing_directory():
    ...
    with runner.isolated_filesystem():
        ...
        assert result.exit_code == 0
        assert f"Directory '{project_name}' already exists. Try -f to force overwrite." in result.output
...
def test_new_command_force_overwrite():
    ...
    with runner.isolated_filesystem():
        ...
        os.mkdir(project_name)
        (Path(project_name) / "existing_file.txt").write_text("This file should be removed")
